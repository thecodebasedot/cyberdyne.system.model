...
class Telemetry(Module):
    ...
    def build(self) -> dict:
        ...
        snap = {
            ...
            "routines": ctx.extras["routines"].status() if "routines" in ctx.extras else [],
            ...
            "peers": bus.latest_payload("fleet/peers", []),
            "auction": ctx.extras["auction"].status() if "auction" in ctx.extras else None,
            "ops": ctx.extras["ops"].describe() if "ops" in ctx.extras else None,
